...
5. **[USER]** Fill in `/etc/arcemx.env` with the real values (same 8 vars as Render's env, see `deploy/oracle/arcemx.env.template` for the exact list), then `sudo systemctl restart arcemx-bot`.
