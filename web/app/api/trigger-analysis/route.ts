// Proxy to bot's /trigger/analysis. Bot token never reaches browser.
// Runs the LLM analysis pipeline only (no INDmoney sync, no grader).
...
export async function POST(req: NextRequest) {
  ...
  try {
    const r = await fetch(`${botUrl.replace(/\/$/, "")}/trigger/analysis`, {
      method: "POST",
      headers: {
